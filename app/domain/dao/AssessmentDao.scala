...
trait AssessmentDao {
  ...
  def getFinishedAssessmentsWithSittingInformation(department: Option[DepartmentCode], examProfileCode: Option[String], importedOnly: Boolean): DBIO[Seq[(StoredAssessment, Set[StoredUploadedFile], Set[(StoredStudentAssessment, Option[StoredDeclarations], Set[StoredUploadedFile])])]]
}
...
class AssessmentDaoImpl @Inject()(
  ...
  override def getFinishedAssessmentsWithSittingInformation(department: Option[DepartmentCode], examProfileCode: Option[String], importedOnly: Boolean): DBIO[Seq[(StoredAssessment, Set[StoredUploadedFile], Set[(StoredStudentAssessment, Option[StoredDeclarations], Set[StoredUploadedFile])])]] =
    pastLastSubmitTimeQuery
      .filter { a =>
        ...
        (if (examProfileCode.nonEmpty) a.examProfileCode === examProfileCode.get else true.bind)
      }
      .withUploadedFiles
      .joinLeft(
