...
trait VersionedTables {
  self: HasDatabaseConfigProvider[_ <: JdbcProfile] =>
  ...
  object VersionedTableQuery {
    def apply[
      A <: Versioned[A],
      B <: StoredVersion[A],
      C <: Table[A] with VersionedTable[A],
      D <: Table[B] with StoredVersionTable[A]
    ](
      table: TableQuery[C],
      versionsTable: TableQuery[D]
    ) =
  ...
  class VersionedTableQuery[A <: Versioned[A], B <: StoredVersion[A], C <: Table[A] with VersionedTable[A], D <: Table[B] with StoredVersionTable[A]](
    ...
    def insertOrUpdate(value: A)(implicit ec: ExecutionContext, ac: AuditLogContext): DBIO[A] = {
      table.filter(_.matchesPrimaryKey(value))
        .result
        .headOption
        .flatMap {
          // Existing row which differs (ignoring the version)
          case Some(existing) if existing != value.atVersion(existing.version) => update(value.atVersion(existing.version))

          // Identical
          case Some(existing) => DBIO.successful(existing)

          // No existing row
          case None => insert(value)
        }
    }
    ...
    private[this] def optimisticLockingException(value: A)(implicit ec: ExecutionContext): DBIO[Nothing] =
      table.filter(_.matchesPrimaryKey(value)).result.flatMap {
        case Seq(current) => DBIO.failed(new Exception(s"Optimistic locking failed - tried to update version ${value.version} but current value is ${current.version} for class ${value.getClass}"))
    ...
    def updateAll(values: Seq[A])(implicit ec: ExecutionContext, ac: AuditLogContext): DBIO[Seq[A]] = DBIO.sequence(values.map(update))
    ...
    def history[T <: slick.lifted.Rep[_]](idFilter: D => T)(implicit wt: CanBeQueryCondition[T]): DBIO[Seq[B]] =
      this.versionsTable
        .filter(idFilter)
        .filter(e =>
