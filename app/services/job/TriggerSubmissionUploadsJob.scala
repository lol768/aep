...
import domain.{AssessmentMetadata, JobKeys}
    ...
    if (configuration.get[Boolean]("tabula.postSubmissions")) {
      ...
      assessmentService.getFinishedWithUnsentSubmissions.successMapTo { assessments =>
        ...
        assessments.map { assessmentMetadata =>
          ...
          val uploadAssessmentJobfailed = scheduler.checkExists(errorKey)
          if (uploadAssessmentJobfailed) {
          ...
          UploadAssessmentInfo(assessmentMetadata, uploadAssessmentJobfailed)
        }
      }.map(_.fold(
        errors => {
