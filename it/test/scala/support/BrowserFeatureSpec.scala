...
abstract class BrowserFeatureSpec extends AbstractFunctionalTest
  ...
  object steps {
    ...
    def i_have_a_moodle_assignment_to_take(student: User): Unit = {
      ...
      val brief = execWithCommit(uploadedFileService.storeDBIO(Fixtures.uploadedFiles.specialJPG.byteSource, Fixtures.uploadedFiles.specialJPG.uploadedFileSave.copy(fileName = "Brief.pdf"), Usercode("thisisfine"), assessment.id, UploadedFileOwner.AssessmentBrief))
